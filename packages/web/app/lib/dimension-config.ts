...
export const DIMENSIONS: Record<string, DimensionDef> = {
  ...
  HTF: {
    ...
    extractMetrics: (ctx) => [
      ...
      { label: "Futures CVD", value: safe(() => `${formatNumber(get(ctx, "cvd.futures.value") as number, 0)} — ${get(ctx, "cvd.futures.short.regime")}/${get(ctx, "cvd.futures.long.regime")}${get(ctx, "cvd.futures.divergence") !== "NONE" ? ` [${get(ctx, "cvd.futures.divergence")} div]` : ""}`) },
      { label: "Spot CVD", value: safe(() => `${formatNumber(get(ctx, "cvd.spot.value") as number, 0)} — ${get(ctx, "cvd.spot.short.regime")}/${get(ctx, "cvd.spot.long.regime")}${get(ctx, "cvd.spot.divergence") !== "NONE" ? ` [${get(ctx, "cvd.spot.divergence")} div]` : ""}`) },
